...
class IPv4:
    ...
    def __init__(self, buffer: bytes):
        ...
        src = b[96:128]
        dst = b[128:160]
        self.src = f"{int(src[0:8], 2)}.{int(src[8:16], 2)}.{int(src[16:24], 2)}.{int(src[24:32], 2)}"
        self.dst = f"{int(dst[0:8], 2)}.{int(dst[8:16], 2)}.{int(dst[16:24], 2)}.{int(dst[24:32], 2)}"
    ...
    for ttl in range(1, TRACEROUTE_MAX_TTL+1):
        ...
        for _ in range(PROBE_ATTEMPT_COUNT):
            ...
            if recvsock.recv_select():
                ...
                if addr[0] not in route_ip:
                    route_ip.append(addr[0])
                    if addr[0] == ip:
                        res.append(route_ip)
                        util.print_result(route_ip, ttl)
                        return res
        res.append(route_ip)
        util.print_result(route_ip, ttl)
    return res
